...
def build_svr_model(C: float, epsilon: float, gamma: str = "scale") -> MultiOutputRegressor:
    base_model = Pipeline(
        steps=[
...
def tune_svr(
    ...
    Small grid so smoke test remains manageable.
    """
    param_grid = {
        "C": [1.0, 10.0],
        "epsilon": [0.01, 0.1],
...
def rolling_recalibration_forecast(
    ...
    for i in range(n_test_days):
        ...
        if (i + 1) % 5 == 0 or i == 0 or (i + 1) == n_test_days:
...
def save_outputs(
    ...
    forecast_path = out_dir / f"SVR_rolling_forecast_{market}_{suffix}.csv"
    metrics_path = out_dir / f"SVR_rolling_metrics_{market}_{suffix}.json"
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["smoke", "final"],
        default="smoke",
        help="smoke = first 3 test days, final = full test period",
    )
    parser.add_argument(
        "--out-dir",
        default=None,
    ...
    best_params = tune_svr(x_train_core, y_train_core, x_val, y_val)
